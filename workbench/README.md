...
- Reviewed text patches up to 250 KB, transported within a 50 KB input budget using bounded compression when needed. The approval hashes the original patch bytes, and the isolated runner verifies the exact reproduced source tree. Submodules, binary patches, renames, framework migrations, installers, and updater redesign require a different execution profile.
...
`record-device --report <scenario-report.json>` imports package-bound scenario assertions and independently inspects the still-running process using `GetProcessInformation(ProcessMachineTypeInfo)` and the executable's PE header. `IsWow64Process2` is retained only as supplemental host/WOW64 evidence; x64 emulation on ARM64 must not be inferred from that API alone. The report must contain the current `candidateBuild` object as `candidate`, `packageTreeHash`, `processId`, the process's UTC `processStartedAt`, and named, passing `scenarios`. The executable path, loaded package runtime modules, module hashes, and process instance must match. Scenario assertions remain attributable to the supplied harness; architecture is independently re-measured.
...
When a coding session finished but controller finalization was interrupted, an operator can complete cumulative review, approve the preserved `interrupted-port` hash, then use `finalize <run-id> --reviewed-tree-hash <hash>`. This creates a fresh build-patch approval using the currently configured protected runner. It neither executes the source nor claims the candidate works. The original unchanged-baseline plan and receipt remain intact.
